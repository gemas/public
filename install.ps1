...
function Invoke-WinsInstaller {
    ...
    function Test-RancherConnection {
        ...
        if ($env:CATTLE_SERVER -and ($env:CATTLE_REMOTE_ENABLED -eq "true")) {
            $retries = 0  
            while ($retries -lt 6) {
                $responseCode = $(curl.exe --insecure --connect-timeout 60 --max-time 60 --write-out "%{http_code}\n" $env:CURL_CAFLAG -sfL "$env:CATTLE_SERVER/healthz")
                switch ( $responseCode ) {
                    { $_ -in "ok200", 200 } {
    ...
    function Test-CaRequired {
        ...
        if ($env:CATTLE_SERVER -and ($env:CATTLE_REMOTE_ENABLED -eq "true")) {
            $retries = 0  
            while ($retries -lt 6) {
                curl.exe  --insecure --connect-timeout 60 --max-time 60 -sfL "$env:CATTLE_SERVER/healthz"
                switch ($LASTEXITCODE) {
                    0 {
...
$env:CATTLE_AGENT_BINARY_BASE_URL="https://rancher.local/assets"
$env:CATTLE_SERVER="https://rancher.local"
$env:CATTLE_CA_CHECKSUM="f33184a8c725553c53fad76f3e8a59d4a5f5af159ebfa6e691a56f0df8bec321"


# Enables CSI Proxy
$env:CSI_PROXY_URL = "https://rancher.local/assets/csi-proxy-%[1]s.tar.gz"
$env:CSI_PROXY_VERSION = "v1.1.1"
